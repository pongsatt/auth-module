import { encodeQuery, parseQuery, randomString, decodeIdToken } from '../utilities'
...
export default class Oauth2Scheme {
  ...
  isIdToken () {
    return this.options.response_type.includes('id_token')
  }
  ...
  login () {
    ...
    if (this.isIdToken()) {
      opts.nonce = randomString()
      this.$auth.$storage.setLocalStorage(this.name + '.nonce', opts.nonce)
    }
  ...
  async fetchUser () {
    ...
    if (this.isIdToken()) {
      const jwtDecoded = decodeIdToken(token.split(' ')[1], this.domain)
      ...
      // Validate nonce
      // TODO remove nonce universally or on browser in callback
      const nonce = this.$auth.$storage.getLocalStorage(this.name + '.nonce')
      this.$auth.$storage.setLocalStorage(this.name + '.nonce', null)
      if (nonce && jwtDecoded.payload.nonce !== nonce) {
        this.$auth.setUser({})
        return
      }

      this.$auth.setUser(jwtDecoded.payload)
      return
    }
